...
public class Dictionary {
  /*
   * 'words' is not only private but DO NOT have getters (like getWords()). This
   * prevents callers from grabbing the words and potentially manipulating them.
   * contains() lets callers check membership, and that's good enough.
   *
   * If you felt it was needed, how would you provide safe access to all of the
   ...
   *
   * @param input
   *          The words to put in the dictionary
   */
  ...
  /**
   * Reports on whether a string is a prefix of any word in this Dictionary. A
   * word is a prefix of itself.
   *
   * @param pref
   *          The string to test
   * @return true if pref is the beginning of some word in this Dictionary, else
   *         false.
   */
  public boolean containsPrefix(String pref) {
    // Get a sorted set of all words after 'pref'
    SortedSet<String> tail = words.tailSet(pref);
    // if there are no such words, then the prefix is not contained
    if (tail.size() == 0) {
      return false;
    }
    // check if the first word greater than pref starts with pref itself
    return tail.first().startsWith(pref);
  }
}
